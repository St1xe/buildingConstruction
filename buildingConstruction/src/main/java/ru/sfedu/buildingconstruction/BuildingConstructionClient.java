    ...
    public static void main(String[] args) throws Exception {
        ...
        switch (arguments[0]) {
            ...
            case "-worker" -> {
                ...
                if (EntityConfugurationUtil.phoneNumberIsCorrect((arguments[3]))) {
                    worker.setPhoneNumber(arguments[3]);
                } else {
                    return;
                }
                if (EntityConfugurationUtil.numberIsPositive(Double.parseDouble(arguments[4]))) {
                    worker.setSalary(Double.parseDouble(arguments[4]));
                } else {
                    return;
                }
            ...
            case "-plan" -> {
                ...
                switch (arguments[1]) {
                    ...
                    case "ApartmentHouse" -> {
                        ...
                        ((ApartmentHouse) building).setNumberOfApartments(Integer.parseInt(arguments[3]));
                    }
                    case "House" -> {
                        building = new House();
                        ((House) building).setNumberOfRooms(Integer.parseInt(arguments[3]));
                    }
                    case "Garage" -> {
                        building = new Garage();
                        ((Garage) building).setNumberOfCars(Integer.parseInt(arguments[3]));
                    }
                    default -> {
                        log.error("Данные введены некорректно");
                        return;
                ...
                Arrays.stream(arguments[6].split(",")).forEach(el -> {

                    try {
                        list.add(provider.getMaterial(el).get());
                ...
                Arrays.stream(arguments[7].split(",")).forEach(el -> {

                    systems.add(EngineeringSystem.valueOf(el));
