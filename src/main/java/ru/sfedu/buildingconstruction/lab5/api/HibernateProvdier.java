...
public class HibernateProvdier {

    private static final Logger log = Logger.getLogger(HibernateProvdier.class);
    ...
    public void deleteAllRecordsFromTable() {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            session.createMutationQuery(Constants.DELETE_MATERIAL_RECORDS).executeUpdate();
            session.createMutationQuery(Constants.DELETE_CONSTRUCTION_EQUIPMENT_RECORDS).executeUpdate();
            session.createMutationQuery(Constants.DELETE_WORKER_RECORDS).executeUpdate();
            session.createMutationQuery(Constants.DELETE_BUILDING_RECORDS).executeUpdate();
            session.createMutationQuery(Constants.DELETE_CLIENT_RECORDS).executeUpdate();
